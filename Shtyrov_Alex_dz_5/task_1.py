...
def calc():
    ...
    for i in range(n):
        ...
        profit_all[company.name] = (company.period_1 + company.period_2 +
                                    company.period_3 + company.period_4)

    total_aver = 0
    for value in profit_all.values():
        total_aver += value
    total_aver = total_aver / n
    print(f"Средняя годовая прибыль всех предприятий: {total_aver}")
    for key, value in profit_all.items():
        if value > total_aver / n:
            print(f"Предприятия, с прибылью выше среднего значения: {key}.")
        elif value < total_aver / n:
